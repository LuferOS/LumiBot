  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (!name || name.length < 4 || name.length > 64) {
        return m.reply('💙 El nombre del paquete de stickers debe tener entre 4 y 64 caracteres.', m, global.miku)
      }
      ...
      m.reply(`💙 El paquete de stickers \`${name}\` ha sido creado exitosamente!\n> Puedes agregar stickers respondiendo a uno usando *${usedPrefix}addsticker ${name}*!`, m, global.miku)
    } catch (e) {
