export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      if (!args.length) {
        return m.reply(`✨ Especifica el nombre del paquete y la nueva descripción.\n> Ejemplo: *${usedPrefix + command} NombreDelPaquete | Nueva Descripción*`, m, global.lumi)
      }
      ...
      if (!desc || desc.length === 0) {
        return m.reply('✨ La descripción no puede estar vacía.', m, global.lumi)
      }
      if (desc.length > 60) {
        return m.reply('💅 La descripción no puede tener más de 60 caracteres.', m, global.lumi)
      }
