  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (pack.spackpublic === 1) {
        return m.reply(`💙 El paquete de stickers \`${pack.name}\` ya es público.`, m, global.miku)
      }
      pack.spackpublic = 1
      pack.lastModified = Date.now().toString()
      db.stickerspack[m.sender].packs = packs
      m.reply(`💙 El paquete de stickers \`${pack.name}\` ha sido establecido como público!`, m, global.miku)
    } catch (e) {
