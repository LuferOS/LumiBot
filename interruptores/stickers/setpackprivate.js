export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      if (!args.length) {
        return m.reply('💙 Debes especificar el nombre del paquete de stickers.', m, global.miku)
      }
      ...
      if (pack.spackpublic === 0) {
        return m.reply(`💙 El paquete de stickers \`${pack.name}\` ya es privado.`, m, global.miku)
      }
      pack.spackpublic = 0
      pack.lastModified = Date.now().toString()
      db.stickerspack[m.sender].packs = packs
      m.reply(`💙 El paquete de stickers \`${pack.name}\` ha sido establecido como privado!`, m, global.miku)
    } catch (e) {
