...
export default {
  ...
  run: async (client, m, args) => {
   ...
   const userId = await resolveLidToRealJid(who2, client, m.chat);
    ...
    if (total === 0) {
      return client.reply(m.chat, `✨ @${userId.split('@')[0]} no tiene advertencias registradas.`, m, global.lumi, { mentions: [userId] })
    }
